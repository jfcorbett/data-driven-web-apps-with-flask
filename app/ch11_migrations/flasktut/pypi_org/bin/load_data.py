...
def find_users(data: List[dict]) -> dict:
    ...
    with progressbar.ProgressBar(max_value=len(data)) as bar:
        for idx, p in enumerate(data):
            ...
            found_users.update(
                get_email_and_name_from_text(info.get('maintainer'), info.get('maintainer_email')))
            bar.update(idx)
...
def load_package(data: dict, user_lookup: Dict[str, User]):
    try:
        ...
        maintainers_lookup = get_email_and_name_from_text(info.get('maintainer'),
